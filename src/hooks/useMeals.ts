...
export const useMeals = () => {
  ...
  const addMeal = useMutation({
    ...
    onSuccess: (_, variables) => {
       ...
       notifyPartner("meals", "Plan posiłków", `🍽️ ${variables.name} — ${days[variables.dayOfWeek]}`, "🍽️");
     },
    onError: (error) => {
  ...
  const addMealFromFavorite = useMutation({
    ...
    onSuccess: (_, variables) => {
       ...
       const days = ["Poniedziałek", "Wtorek", "Środa", "Czwartek", "Piątek", "Sobota", "Niedziela"];
       notifyPartner("meals", "Nowy przepis", `🍽️ ${variables.name} — ${days[variables.dayOfWeek]}`, "🍽️");
     },
    onError: (error) => {
